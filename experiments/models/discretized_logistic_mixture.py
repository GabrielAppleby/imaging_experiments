...
class DiscMixLogistic:
    def __init__(self, param, device, num_mix=10, num_bits=8):
        B, C, H, W = param.size()
        self.device = device
    ...
    def log_prob(self, samples):
        ...
        samples = samples.expand(-1, -1, -1, -1, self.num_mix).permute(0, 1, 4, 2,
                                                                       3)  # B, 3, M, H, W
    ...
    def sample(self, t=1.):
        gumbel = -torch.log(- torch.log(
            torch.Tensor(self.logit_probs.size()).uniform_(1e-5, 1. - 1e-5).to(self.device)))  # B, M, H, W
        sel = one_hot(torch.argmax(self.logit_probs / t + gumbel, 1), self.num_mix, 1, self.device)  # B, M, H, W
        ...
        u = torch.Tensor(means.size()).uniform_(1e-5, 1. - 1e-5).to(self.device)  # B, 3, H, W
        ...
        x2 = torch.clamp(x[:, 2, :, :] + coeffs[:, 1, :, :] * x0 + coeffs[:, 2, :, :] * x1, -1,
                         1)  # B, H, W
